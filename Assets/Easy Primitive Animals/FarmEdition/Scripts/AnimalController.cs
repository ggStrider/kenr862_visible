...
namespace EasyPrimitiveAnimals
{
    public class AnimalController : MonoBehaviour
    {
        ...
        private bool _wasPushedByPlayerOnce;

        [Space] [SerializeField] private string _tutorialTextOnCollideByPlayerOnce =
            "Корова змінює свій кут шляху на 90 градусів, коли стикається з перешкодою";
        ...
        {
            if (!_wasPushedByPlayerOnce && collision.gameObject.CompareTag(StaticKeys.PLAYER_TAG))
            {
                _wasPushedByPlayerOnce = true;
                _tutorialTextThrower.ThrowTutorial(_tutorialTextOnCollideByPlayerOnce);
            }
            if (!collision.gameObject.CompareTag("Carpet") &&
                !collision.gameObject.CompareTag("WoodenFloor") && canRotate) // If the animal collides with something that is not the ground, spin it around.
            {
